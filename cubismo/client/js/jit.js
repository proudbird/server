  ...
  defaults:{
    ...
    on:{
      ...
      onItemClick: function(id, e, node) {
        ...
        for (key in columns) {
          if(columns[key].id == id.column) {
            if(columns[key].dataLink) {
              showLookup(this, id, e, node, columns[key].dataLink);
            }
          }
        }
      },
    ...
    on:{
      ...
      onItemDblClick: function(item) {
        ...
        if (id) {
          ...
          if(form.config && form.config.purpose && form.config.purpose == "lookup") {
            var message = {};
            message.FormID    = this.config.formID;
            message.Command   = "afterLookup";
            message.view      = { id: this.config.id, name: this.config.name };
            message.Value     = this.getItem(id);
            ServerCall(message);
          } else {
            var message = {};
...
socket.on('message', function (msg, callback) {
    ...
    try {
      if(Array.isArray(msg.data)) {
        for(let i=0; i<msg.data.length; i++) {
          $$(msg.viewID).remove(msg.data[i]);
        };
      } else {
        $$(msg.viewID).remove(msg.data);
      }
    } catch(error) {
      console.log(error);
      callback(error);
  ...
  else if(msg.directive == 'updateItem') {
    let view = $$(msg.target.id);
    var item = view.getItem(msg.target.rowId);
    item[msg.target.columnId] = msg.value;
    view.updateItem(msg.target.rowId, item);
    callback(null);
  }
  else if(msg.directive == 'updateInstance') {
    let view = $$(msg.target.id);
    view.setValue(msg.value);
    callback(null);
  }
...
webix.editors.lookup = webix.extend({
  render:function(){
    var inBox = document.createElement("div");
    inBox.className = "webix_el_box";
    var input = document.createElement("input");
    inBox.appendChild(input);

    inBox.className = "input_buttons_box";
    //input.style.top = "-" + height + "px";
    ...
    inButton.addEventListener("click", function() {            
      var message = {};
      message.FormID    = self.config.formID;
      message.Command   = "LookupForAttribute";
      ServerCall(message);
    });
  ...
  //console.log(webix.ui.segmented);